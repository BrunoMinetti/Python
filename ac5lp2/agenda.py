...
#
# e-mails: bruno.sanches@aluno.faculdadeimpacta.com.br
...
class Email:
    ...
    def valida_email(email: str) -> bool:
        ...
        if not isinstance(email, str):
            raise TypeError
        elif '_' in email or email.count('@') != 1 or email.replace('.', '').isalnum():
            raise ValueError
        else:
            return True
...
class Contato:
    ...
    def lista_telefones(self) -> List[Tuple[str, Telefone]]:
        ...
        """
        lista_telefones = []
        for tipo, telefone in self._telefones.items():
            lista_telefones.append((tipo, telefone))
    ...
    def lista_emails(self) -> List[Tuple[str, Email]]:
        ...
        """
        lista_emails = []
        for tipo, email in self._emails.items():
            lista_emails.append((tipo, email))
        return lista_emails
    ...
    def buscar(self, valor_busca: str):
        ...
        for chave, valor in self._telefones.items():
            if valor_busca in valor.telefone:
                return True

        for chave, valor in self._emails.items():
            if valor_busca in valor.email:
                return True
...
class Agenda:
    ...
    def ligar(self, valor_busca, tipo='principal') -> None:
        ...
        """
        for contato in self.busca_contatos(valor_busca.lower()):
            for atributo in contato.lista_telefones():
                if tipo == atributo[0]:
                    return f'Ligando para {contato.nome}: {atributo[1]}'
    ...
    def exportar_contatos(self, nome_arquivo: str) -> None:
        ...
        lista = []
        for contato in self.contatos:
            lista.append(contato.create_dump())
        registro = open(nome_arquivo, 'w')
        registro.write(json.dumps(lista, default=dumper, indent=4, ensure_ascii=False))
        registro.close()
    ...
    def carregar_contatos(self, nome_arquivo: str) -> None:
        ...
        if not nome_arquivo.endswith('.json'):
            nome_arquivo += '.json'
        with open(nome_arquivo,'w') as registro:
            contatos = json.loads(registro)
